...
public class ChatClient {
            ...
            new Thread(() -> {
                try {
                    ...
                    while (true) {
                        ...
                        byte []  localStoryFileReceived = message.getBytes();
                        try (FileOutputStream out = new FileOutputStream("localFile",true)) {
                            out.write(localStoryFileReceived);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
    ...
    public void sendMessage(String message) {
        try {
            ...
            byte []  localStoryFileSend = message.getBytes();
                try (FileOutputStream out = new FileOutputStream("localFile.txt",true)) {
                    out.write(localStoryFileSend);
                } catch (IOException e) {
                e.printStackTrace();
                }
