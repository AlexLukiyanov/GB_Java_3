    ...
    public void run() {
        try (ServerSocket serverSocket = new ServerSocket(8189)) {
            while (true) {
                ...
                JDBCAuthorization jdbcAuthorization = new JDBCAuthorization();
                try {
                    jdbcAuthorization.connect();
                    jdbcAuthorization.createTable();
                    jdbcAuthorization.insert("alexander", "123alex123");
                    jdbcAuthorization.insert("frank", "123frank123");
                    jdbcAuthorization.insert("ivanov", "123ivanov123");
                    jdbcAuthorization.insert("petrov", "123petrov123");
                    System.out.println(jdbcAuthorization.chekAccount("alexander", "123alex123"));
                    jdbcAuthorization.dropTable();
                } catch (SQLException e) {
                    e.printStackTrace();
                } finally {
                    jdbcAuthorization.disconnect();
                }
